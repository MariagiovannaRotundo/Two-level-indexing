...
int main(int argc, char* argv[]) {
    ...
    if(!std::filesystem::exists(fresult)){ //if the file fo not exists
        ...
        fprintf(f,"name,dataset,logical_b_size,avg_time,space,disk_space,construction_time,dataset_query,space_cache,page_fault,repetitions,pages_in_cache,tot_pages,max_cache,randomIOs,time_no_scan\n");// newline
    }
    else{
        f = std::fopen(output_file.data(), "a");
        if(!f) {
